...
console.log('🕋 100% HALAL EXNESS TRADING BOT - REAL ONLY');
console.log('📦 Version: 30.0.0');
console.log('✅ NO SIMULATION - REAL EXNESS DATA ONLY');
console.log('✅ Will show error if balance cannot be fetched');
console.log('✅ 100% HALAL - No leverage, no interest, no gambling');
console.log('🔥 AGGRESSIVE - Trades every 2 seconds');
...
// ==================== REAL BALANCE FETCH - NO SIMULATION ====================
async function fetchRealBalance(accountId) {
    try {
        // Step 1: Check if TickerAll is initialized
        if (!ticker) {
            console.error('❌ TickerAll not initialized!');
            return { balance: null, currency: 'USD', error: 'TickerAll not initialized', isReal: false };
        }
        
        // Step 2: Check if we have an account ID
        if (!accountId) {
            console.error('❌ No account ID!');
            return { balance: null, currency: 'USD', error: 'No account ID', isReal: false };
        }

        console.log(`🔍 Fetching REAL balance from Exness for session: ${accountId}`);
        
        // Step 3: Fetch account info from TickerAll (REAL DATA)
        ...
        if (!accountInfo) {
            ...
            return { balance: null, currency: 'USD', error: 'No account info received', isReal: false };
        }

        console.log('📊 REAL account info received from Exness:');
        console.log(JSON.stringify(accountInfo, null, 2));

        // Step 4: Extract balance from the REAL data
        let balance = null;
        let currency = accountInfo.currency || accountInfo.Currency || 'USD';

        // Try all possible field names that Exness might use
        const balanceFields = [
        ...
        // If still null, scan ALL numeric fields (but don't simulate)
        if (balance === null) {
            console.log('🔍 Scanning all numeric fields for balance...');
            for (const [key, value] of Object.entries(accountInfo)) {
                if (typeof value === 'number' && !isNaN(value) && value < 100000000) {
                    // Check if the key suggests it's a balance field
                    const keyLower = key.toLowerCase();
                    if (keyLower.includes('balance') || keyLower.includes('equity') || 
                        keyLower.includes('margin') || keyLower.includes('free') ||
                        keyLower.includes('funds') || keyLower.includes('cash') ||
                        keyLower.includes('total') || keyLower.includes('amount') ||
                        keyLower.includes('available') || keyLower.includes('usable')) {
                        balance = value;
                        foundField = key;
        ...
        // Step 5: If still null, return error - DO NOT SIMULATE
        if (balance === null) {
            console.error('❌ Could not find balance in any field!');
            return { 
                balance: null, 
                currency: 'USD', 
                error: 'Could not find balance in account data', 
                isReal: false,
                fullData: accountInfo
            };
        }

        // Step 6: Store the real balance in user data
        const users = readUsers();
        ...
        console.log(`✅ Balance is REAL from Exness, NOT simulation`);
...
async function verifyExnessConnection(accountId) {
    try {
        if (!ticker) return { success: false, error: 'TickerAll not initialized' };
        if (!accountId) return { success: false, error: 'No account ID' };

        console.log(`🔍 Verifying Exness connection for session: ${accountId}`);
        ...
        if (!accountInfo) {
            return { success: false, error: 'No account info returned' };
        }

        const hasRealData = accountInfo.balance !== undefined || 
                           accountInfo.equity !== undefined || 
                           accountInfo.currency !== undefined;
...
app.get('/api/admin/user-balances', authenticate, async (req, res) => {
    ...
    for (const [email, userData] of Object.entries(users)) {
        if (!userData.tickerallSessionId) {
            balances[email] = { balance: null, hasConnection: false, isReal: false };
            continue;
        }
        try {
            ...
            balances[email] = { balance: null, hasConnection: false, error: error.message, isReal: false };
        }
    }
...
app.post('/api/set-exness-creds', authenticate, async (req, res) => {
    try {
        ...
        console.log(`📊 Connecting to REAL Exness for user: ${req.user.email}`);
        ...
        // Step 2: Verify connection is real
        const verification = await verifyExnessConnection(accountId);
        if (!verification.success || !verification.isReal) {
            console.error('❌ Connection is not real!');
            return res.status(401).json({ 
                success: false, 
                message: 'Could not verify real Exness connection. Please check your credentials.'
            });
        }

        // Step 3: Fetch real balance
        const result = await fetchRealBalance(accountId);
        
        if (result.balance === null) {
            return res.status(401).json({ 
                success: false, 
                message: `Could not fetch balance from Exness: ${result.error || 'Unknown error'}`
            });
        }
        ...
        res.json({
            success: true,
            message: `✅ Connected to REAL Exness! Balance: ${result.balance} ${result.currency || 'USD'}`,
            balance: result.balance,
            currency: result.currency || 'USD',
            isReal: true,
            accountId: accountId
...
app.post('/api/connect-exness', authenticate, async (req, res) => {
    try {
        ...
        const verification = await verifyExnessConnection(user.tickerallSessionId);
        if (!verification.success || !verification.isReal) {
            return res.status(401).json({ 
                success: false, 
                message: 'Connection is not real. Please reconnect with valid Exness credentials.'
            });
        }
        ...
        res.json({
            ...
            currency: result.currency || 'USD',
            isReal: true,
            message: `Connected to REAL Exness! Balance: ${result.balance || 0} ${result.currency || 'USD'}`
        });
    } catch (error) {
...
app.get('/api/debug-balance', authenticate, async (req, res) => {
    try {
        ...
        if (!user || !user.tickerallSessionId) {
            return res.json({ 
                success: false, 
                message: 'No session ID found. Please connect to Exness first.'
            });
        }
        ...
        res.json({
            ...
            currency: result.currency || 'USD',
            isReal: result.isReal || false,
            connectionVerified: verification.success && verification.isReal,
...
function calculateMACD(prices, fast = 12, slow = 26, signal = 9) {
    ...
    const signalLine = prices.slice(-signal).reduce((a, b) => a + b, 0) / signal - 
                       (prices.slice(-slow, -slow + signal).reduce((a, b) => a + b, 0) / signal);
...
async function getHalalAIDecision(symbol, accountId) {
    try {
        ...
        if (!rates || rates.length < 30) {
            return { action: 'BUY', confidence: 0.5, reasons: ['Insufficient data - default BUY'], currentPrice: 0 };
        }
        ...
        if (buyScore > sellScore) {
            action = 'BUY';
            confidence = Math.min(0.9, 0.5 + (buyScore / (buyScore + sellScore)) * 0.4);
        } else if (sellScore > buyScore) {
            action = 'SELL';
            confidence = Math.min(0.9, 0.5 + (sellScore / (buyScore + sellScore)) * 0.4);
        } else {
            if (currentPrice > ma20) { action = 'BUY'; confidence = 0.55; }
            else { action = 'SELL'; confidence = 0.55; }
        }
        ...
        return { action: 'BUY', confidence: 0.5, reasons: ['AI error - default BUY'], currentPrice: 0 };
    }
}
...
class HalalTradingEngine {
    ...
    async start() {
        console.log(`🕋 Starting HALAL AI trading for ${this.userEmail}`);
        ...
        setTimeout(() => {
            if (this.isActive && this.openPositions.length === 0) {
                console.log('🔥 IMMEDIATE FORCE TRADE!');
                this.forceOpenTrade();
            }
        }, 3000);
    ...
    async forceOpenTrade() {
        ...
        try {
            ...
            if (action === 'HOLD') {
                ...
                aiDecision.reasons = ['Force BUY - Halal trading'];
            }
            console.log(`🔥 HALAL FORCE TRADE: ${action} on ${symbol}`);
    ...
    async executeTrade(symbol, side, aiDecision) {
        ...
        try {
            ...
            if (result.balance === null) {
                console.error(`❌ Cannot trade: ${result.error || 'No balance available'}`);
                return;
            }
            ...
            const positionSize = Math.min(balance / maxPositions, balance * 0.35);
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            sessionId,
            message: `🕋 HALAL AI TRADING STARTED! Investment: $${investmentAmount} | Target: $${targetProfit} | NO LEVERAGE - 1:1 Spot Trading | 🤖 AI analyzes EVERY 2 SECONDS!`
        });
    } catch (error) {
